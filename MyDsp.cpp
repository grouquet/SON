...
void MyDsp::update(void) {
    ...
    for (int i = 0; i < AUDIO_BLOCK_SAMPLES; i++) {
        ...
        float inputLevelDb = 20 * log10(abs(currentSample) + 1e-6); // Convertir en dB, éviter log(0)
        float compressionGainDb = 0;

        if (inputLevelDb > threshold) {
            float excessDb = inputLevelDb - threshold;
            compressionGainDb = -(excessDb - (excessDb / ratio));
        }

        float compressionGain = pow(10, compressionGainDb / 20);
        currentSample *= compressionGain;
